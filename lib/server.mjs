...
function isIgnoredPathname(pathname) {
  return pathname.split("/").some((part) => part.startsWith(".") || part === "node_modules");
}

export async function createAppServer({ ROOT, PORT, plugins = [], ignoreDotfilesAndModules = true }) {
  ...
  const server = createServer((req, res) => {
    ...
    if (ignoreDotfilesAndModules && isIgnoredPathname(pathname)) {
      res.writeHead(404);
      res.end("Not Found");
      return;
    }
